...
_STRALCIATO_LINE   = "14/25 12345682 VIA ROMA 14 € 50.000,00 € 0,00 STRALCIATO"
_NON_AGGIUD_LINE   = "15/25 12345683 VIA ROMA 15 € 50.000,00 € 0,00 NON AGGIUDICATO"
# NOTE: An empty-winner line (trailing spaces only) is stripped by line.strip() before
# the regex runs, so the regex never matches and the `not winner_clean` branch in
# extract_from_file is unreachable dead code.
...
class TestExtractFromFile:
    ...
    @patch(_PATCH)
    def test_source_pdf_field(
        self, mock_open: MagicMock, extractor: PDFExtractor, tmp_path: Path
    ) -> None:
        mock_open.return_value = _mock_pdf([_AGGIUDICATA_LINE])
        records = extractor.extract_from_file(tmp_path / "test.pdf")
        assert records[0]["source_pdf"] == "test.pdf"
    ...
    def test_null_outcome_keywords(
        ...
        mock_open.return_value = _mock_pdf([line])
        records = extractor.extract_from_file(tmp_path / "test.pdf")
        assert len(records) == 1
        assert records[0]["auction_result"] == expected_result
        assert records[0]["winner"] == ""
    ...
    def test_multi_page_pdf(
        ...
        page1 = MagicMock()
        page1.extract_text.return_value = _AGGIUDICATA_LINE
        page2 = MagicMock()
        page2.extract_text.return_value = _DESERTA_LINE
        pdf_cm = MagicMock()
        pdf_cm.__enter__ = MagicMock(return_value=MagicMock(pages=[page1, page2]))
        pdf_cm.__exit__ = MagicMock(return_value=False)
        mock_open.return_value = pdf_cm

        records = extractor.extract_from_file(tmp_path / "test.pdf")
        assert len(records) == 2
        assert records[0]["auction_result"] == "AGGIUDICATA"
        assert records[1]["auction_result"] == "ASTA DESERTA"
