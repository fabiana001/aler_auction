...
class DatasetIntegrator:
    ...
    def integrate(self, output_path: str) -> Optional[pd.DataFrame]:
        ...
        # Reconcile surface_sqm: prefer Wayback (catastale), fall back to PDF (MQ column)
        if 'surface_sqm_wayback' in df_joined.columns and 'surface_sqm_pdf' in df_joined.columns:
            df_joined['surface_sqm'] = df_joined['surface_sqm_wayback'].fillna(df_joined['surface_sqm_pdf'])
            df_joined.drop(columns=['surface_sqm_wayback', 'surface_sqm_pdf'], inplace=True)
        elif 'surface_sqm_wayback' in df_joined.columns:
            df_joined.rename(columns={'surface_sqm_wayback': 'surface_sqm'}, inplace=True)
        elif 'surface_sqm_pdf' in df_joined.columns:
            df_joined.rename(columns={'surface_sqm_pdf': 'surface_sqm'}, inplace=True)
